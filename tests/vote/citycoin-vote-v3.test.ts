...
import { VoteModelV3 } from "../../models/vote/vote-v3.model.ts";
...
beforeEach(() => {
  ...
  vote = ctx.models.get(VoteModelV3);
});
  ...
  describe("VOTE ACTIONS", () => {
    describe("intialize-contract()", () => {
      it("fails with ERR_UNAUTHORIZED if the contract is already initialized", () => {
        ...
        receipt.result
          .expectErr()
          .expectUint(VoteModelV3.ErrCode.ERR_UNAUTHORIZED);
      });
    ...
    describe("vote-on-proposal()", () => {
      it("fails with ERR_PROPOSAL_NOT_ACTIVE when called before proposal is active", () => {
        ...
        receipt.result
          .expectErr()
          .expectUint(VoteModelV3.ErrCode.ERR_PROPOSAL_NOT_ACTIVE);
      });
      it("fails with ERR_PROPOSAL_NOT_ACTIVE when called after proposal is active", () => {
        ...
        chain.mineEmptyBlockUntil(VoteModelV3.VOTE_END_BLOCK + 1);
        // act
        const receipt = chain.mineBlock([vote.voteOnProposal(true, wallet)])
          .receipts[0];
        // assert
      ...
      it("fails with ERR_NOTHING_STACKED when sender has no stacked tokens", () => {
        ...
        chain.mineEmptyBlock(VoteModelV3.VOTE_START_BLOCK + 1);
        // act
        const receipt = chain.mineBlock([vote.voteOnProposal(true, wallet)])
          .receipts[0];
        // assert
        receipt.result
          .expectErr()
          .expectUint(VoteModelV3.ErrCode.ERR_NOTHING_STACKED);
      });
      it("succeeds with one yes vote when called by a new voter", () => {
        ...
        const miaVote = Math.round(
          ((amountCycle1 * 2 + amountCycle2) / 2) * VoteModelV3.MIA_SCALE_FACTOR
        );
        ...
        chain.mineEmptyBlockUntil(VoteModelV3.VOTE_START_BLOCK + 1);
      ...
      it("fails with ERR_VOTE_ALREADY_CAST when called by an existing voter with the same vote", () => {
        ...
        receipt.result
          .expectErr()
          .expectUint(VoteModelV3.ErrCode.ERR_VOTE_ALREADY_CAST);
      });
    ...
    describe("get-proposals()", () => {
      it("succeeds and returns the proposed CCIPs being voted on", () => {
        ...
        const expectedResult =
          '{hash: "fb1262aec2467c96e683d90e52ef544f724b0a0f", link: "https://github.com/citycoins/governance/blob/main/ccips/ccip-013/ccip-013-stabilize-protocol-and-simplify-contracts.md", name: "Stabilize Protocol and Simplify Contracts"}';
        // act
